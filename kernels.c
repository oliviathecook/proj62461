...
team_t team = {
    ...
    "Olivia Cook",     /* First member full name */
    "oliviacook@gwu.edu",  /* First member email address */
...
/* below are the main computations for your implementation of the rotate. Any changes in implementation will go here or the other functions it may call */
	...
	j = 0;
	i = 0;*/
	/*struct pic *arr = (struct pic*) malloc (sizeof(struct pic));
	arr->a = dst;
	arr->b = src;
	...
	for (j = dim1; j >= 0; j--){
		...
		for (i = dim1; i >= 0; i--){
			//printf("2: %d %d %d\n", dim-1-j, i, j);
			//dst[RIDX(next, i, dim)] = src[RIDX(i, j, dim)];
			arr->a[RIDX(next, i, dim)] = arr->b[RIDX(i, j, dim)];
		}
	}*/
	struct pic** arr = (struct pic**) malloc(sizeof(struct pic*) * dim);
	dim1 = dim-1;
	
	for (i = 0; i < dim; i++){
		arr[i] = (struct pic*) malloc(sizeof(struct pic));
		arr[i]->dst = dst[i];
		arr[i]->src = src[i];
	}
	...
	for (j = dim1; j >= 0; j--){
		...
		for (i = dim1; i >= 0; i--){
			//arr->a[RIDX(next, i, dim)] = arr->b[RIDX(i, j, dim)];
			arr[RIDX(next, i, dim)]->dst = arr[RIDX(i, j, dim)]->src;
